(function() {
	...
	function initGeoLocation() {
		if (navigator.geolocation) { // 检查navigator有没有这个对象
			navigator.geolocation.getCurrentPosition(onPositionUpdated, // 拿到位置
					onLoadPositionFailed, {
						maximumAge : 60000 // cache的最大时间 6分钟
					});
	...
	function getLocationFromIP() {
		...
		ajax('GET', url, data, function(res) {
			var result = JSON.parse(res);
			if ('loc' in result) {
				var loc = result.loc.split(','); // 把 (xxxxx, yyyyyyy) 分开分别拿到lat和lon
				lat = loc[0];
				lng = loc[1];
			} else {
	...
	function register() {
		...
		// 密码加密  在HTML文件（index.html）中引入 md5 library，可以在browser的console里检查是否安装成功。
